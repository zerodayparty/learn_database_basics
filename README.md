...
목표는 단순히 SQL 문장을 외우는 것이 아니다. 아래 흐름을 직접 설명할 수 있는 사람이 되는 것이다.  
...
### 🟡 컨테이너 이름이 이미 있다고 나오는 경우  

```bash
docker rm cafe-postgres  
```

이미 같은 이름의 컨테이너가 남아 있다는 뜻이다. 삭제 후 다시 실행한다.  

### 🟡 5432 포트가 이미 사용 중이라고 나오는 경우  

다른 PostgreSQL이 이미 실행 중일 수 있다.  
...
### 🟡 `createdb`에서 이미 데이터베이스가 있다고 나오는 경우  

이미 만든 데이터베이스가 있다는 뜻이다. 바로 접속하면 된다.  
...
컨테이너 실행할 때 아래 옵션이 빠졌을 가능성이 크다.  

```bash
-v "$(pwd)":/work  
```

이 옵션은 내 Mac의 현재 프로젝트 폴더를 컨테이너 안의 `/work` 폴더로 연결한다.  
